...
def wasserstein_dist(sample,targetsample):
        if isinstance(sample, torch.Tensor):
            sample = sample.numpy()
        if isinstance(targetsample, torch.Tensor):
            targetsample = targetsample.numpy()
        M = ot.dist(sample,targetsample, metric='euclidean')
        Wd = ot.emd2([], [], M)
        return Wd

def statistics(sample,variable_names):
...
def _linear_binning(samples: np.ndarray, grid_points: np.ndarray):
    ...
    try:
        ...
        print("Error occured during computation of histogram counts using a linearly spaced grid.")
        idx = np.floor((samples - (grid_points[0] - 0.5)))
    ...
    **kwargs,
    ...
    for d in range(D):
        ...
        if not np.isinf(lb) and not np.isinf(ub):
            ...
            y_tot = f(grid)
            mtv[d] = 0.5 * simpson(y_tot, x=grid)
        else:
