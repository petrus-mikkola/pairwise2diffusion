...
class r(torch.nn.Module):
    ...
    """r model for log density ratio: log p(x) - log p(x')"""

    def __init__(self, input_dim, hidden_dim):
        super().__init__()
        #Unnormalized log belief density (f_theta in Appendix C.5)
...
class TemperingField:
    ...
    def train_r(self, dataset, batchsize, lr, maxiter, weight_decay=1e-3):
        ...
        for it in tqdm(range(maxiter),disable=True):
            ...
            loss.backward()
            if DEMO: torch.nn.utils.clip_grad_norm_(self.r_model.parameters(), 1.0)
            optimizer.step()
    ...
    def sample_MWD(self):
        ...
        if DEMO: self.log_p_w_X = self.loglik(self.X, lambda x, t: self.MWD_model(x, t, joint=0, temp=0), sigma_min=self.diffusion.sigma_min, sigma_max=self.diffusion.sigma_max,joint_model=False,clamp_log_lik=False, use_hutchinson=True, fresh_noise_hutchinson=True).detach()
    ...
    #Numerically stable version of tau(x)
    def tau(self, x, clamp=True):
        ...
        if not DEMO: w = torch.clamp(w, min=w.quantile(0.01), max = w.quantile(0.9))
        #Analytical tempering field formula under Bradley-Terry model
